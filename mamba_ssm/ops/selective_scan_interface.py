...
def mamba_inner_fn(
    ...
    if selective_scan_cuda is None:
        # Fallback to reference implementation for non-CUDA devices (e.g., Intel XPU)
        return mamba_inner_ref(
            xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
            out_proj_weight, out_proj_bias,
            A, B=B, C=C, D=D, delta_bias=delta_bias, B_proj_bias=B_proj_bias,
            C_proj_bias=C_proj_bias, delta_softplus=delta_softplus,
        )
